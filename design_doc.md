...
(1) We felt that Yujing's design had the best UI out of all possible choices. Her ATM's UI was not only easy to use, but also felt the most fleshed out. Specifically, the handling of certain features such as currencies and bank accounts felt well developed and highly scalable. We felt that this would translate well into designing the UI for the stock market, which will potentially have to display many different stocks. 
...
Overall, we found that our starting design worked out well in the end, but maybe not for the reasons we expected. One of the benefits of our starting design was that the UI and implementation was well fleshed out. However, while this meant the program was easy to use, it did not necessarily mean that it was easy for someone else to get up to speed with the codebase. Because the starting design had well developed features and UI, this also meant that the logic and code was more complex, and required more time to read and understand. On the other hand, having a good starting UI did make it easier to reuse UI elements and functionality for the stock market and investment account windows, which was a great benefit. 

Furthermore, we found that the weakly coupled classes did not really help us split up the work, but having independent classes did make it easier to implement persistent storage via SQLite. Having well decomposed classes made it easier to design our database schema. Our database tables naturally followed from most of the classes, with only a few minor changes needed to cater to the data storage. We also found that we had different opinions on specific implementation details and object structure, but that was to be expected when using another person's implementation.
